...
ROOT = LAYOUT.ubuntu_root
WORKFLOW = LAYOUT.workflow_path
README = ROOT / 'README.md'
ROOT_README = LAYOUT.outer_readme_path


def _missing_markers(text: str, markers: Iterable[str]) -> list[str]:
    """Return all required markers missing from one text payload.

    Args:
        text: Source text to validate.
        markers: Required marker substrings.

    Returns:
        Missing markers in input order.
...
def main() -> int:
    """Run release-gate consistency validation.

    Returns:
        Process exit code.

    Raises:
        SystemExit: When workflow or README markers drift from the manifest.
    """
    ...
    if missing_readme:
        raise SystemExit(f'ubuntu-side README drift: missing markers: {missing_readme}')

    root_expectations = ['./start_frontend.sh', './start_web_bridge.sh', '--config-path', 'canonical-only source release']
    missing_root = _missing_markers(root_readme_text, root_expectations)
    if missing_root:
        raise SystemExit(f'outer README drift: missing markers: {missing_root}')
