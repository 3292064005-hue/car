...
export function createCommandSlice(
  ...
  return {
        ...
        commands: [
          ...
          ...state.commands.map((item) =>
            type === 'teleop_cmd' && item.type === 'teleop_cmd' && (item.status === 'queued' || item.status === 'sent')
              ? { ...item, status: 'superseded' as const, updatedAt: safeNowIso(), error: '被新的 teleop 命令替换' }
              : item,
          ),
        ].slice(0, 100),
      })),
    markCommandSent: (id) =>
      set((state) => ({
        commands: state.commands.map((item) => (item.id === id ? { ...item, status: 'sent', updatedAt: safeNowIso() } : item)),
      })),
      ...
      set((state) => {
        ...
        const ackLatencyMs =
          target && ['ack', 'accepted', 'applied', 'completed', 'rejected', 'denied', 'timeout', 'cancelled'].includes(status)
            ...
            : undefined;
        return {
          commands: state.commands.map((item) => (item.id === id ? { ...item, status, updatedAt: nowIso, error, ackLatencyMs } : item)),
          history:
