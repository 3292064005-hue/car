...
class ActionRuntime:
    ...
    def execute_start_patrol(self, goal_handle: object):
        ...
        if not ok:
            ...
            result.completed_points = int(self._node.context.patrol_index)
        ...
        while rclpy.ok() and not self._shutdown_requested:
            ...
            feedback.step_index = int(self._node.context.patrol_index)
            feedback.completed_points = int(self._node.context.patrol_index)
            feedback.total_points = int(self._node.patrol_manager.total_steps())
            ...
            if self._node.context.patrol_completed:
                ...
                result.final_mode = self._node.current_mode
                result.completed_points = int(self._node.patrol_manager.total_steps())
                result.progress = 1.0
                goal_handle.succeed()
                with self._node._action_lock:
                    self._node._active_patrol_goal = None
