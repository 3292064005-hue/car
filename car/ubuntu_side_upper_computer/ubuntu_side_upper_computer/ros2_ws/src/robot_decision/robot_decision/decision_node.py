...
class DecisionNode(Node):
    ...
    def _mode_guard(self) -> ModeGuard:
        """Resolve the mode-guard helper with compatibility fallback."""
        guard = getattr(self, 'mode_guard', None)
        if guard is None:
            guard = ModeGuard(self)
            try:
    ...
    def _state_controller(self) -> DecisionStateController:
        """Resolve the state-controller helper with compatibility fallback."""
        controller = getattr(self, 'state_controller', None)
        if controller is None:
            controller = DecisionStateController(node=self)
            try:
    ...
    def _side_effects(self) -> DecisionSideEffects:
        """Resolve the side-effect adapter with compatibility fallback."""
        effects = getattr(self, 'side_effects', None)
        if effects is None:
            effects = DecisionSideEffects(node=self)
            try:
    ...
    def _runtime_adapter(self) -> RuntimeParamAdapter:
        """Resolve the runtime-parameter adapter with compatibility fallback."""
        adapter = getattr(self, 'runtime_adapter', None)
        if adapter is None:
            adapter = RuntimeParamAdapter(self)
            try:
    ...
    def _app_service(self) -> DecisionAppService | None:
        return getattr(self, 'app_service', None)
