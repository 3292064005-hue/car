...
def test_run_release_verification_frontend_calls_expected_steps(tmp_path: Path) -> None:
    ...
    subprocess.run([str(script), '--with-frontend', '--skip-npm-ci'], cwd=str(repo_root), env=env, check=True)
    ...
    assert calls[-1] == 'python3:scripts/render_release_quality_manifest.py --output /tmp/release_quality_manifest.json --history-dir /tmp/release_quality_history --common-checks-complete true --frontend-lane true --ros-smoke-lane false --integrated-frontend-smoke-lane false'
...
''',
    ...
    subprocess.run(
        [str(script), '--with-ros-smoke', '--config-path', str(profile_path)],
        cwd=str(repo_root),
        env=env,
        check=True,
    )
...
''',
    ...
    subprocess.run(
        [str(script), '--with-integrated-frontend-smoke', '--skip-npm-ci', '--config-path', str(config_root)],
        cwd=str(repo_root),
        env=env,
        check=True,
    )
    ...
    assert calls[-1] == 'python3:scripts/render_release_quality_manifest.py --output /tmp/release_quality_manifest.json --history-dir /tmp/release_quality_history --common-checks-complete true --frontend-lane true --ros-smoke-lane true --integrated-frontend-smoke-lane true'
