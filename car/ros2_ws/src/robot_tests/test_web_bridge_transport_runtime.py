...
def test_broadcast_queue_full_drops_client_and_reports() -> None:
    async def _run() -> None:
        ...
        assert any(kind == 'queue_full' for kind, _payload in reports)
...
def test_websocket_hub_start_binds_listener_and_serves_initial_snapshot() -> None:
    async def _run() -> None:
        received = []
        reports = []

        async def _command_handler(message: str) -> None:
        ...
        hub = WebSocketHub(
            ...
            port=0,
            ws_path='/ws',
            snapshot_provider=lambda: {'type': 'snapshot', 'payload': {'mode': 'IDLE'}},
            command_handler=_command_handler,
            stats_callback=lambda kind, **payload: reports.append((kind, payload)),
        )
        listener = hub.start(timeout_sec=5.0)
        uri = f"ws://127.0.0.1:{listener['port']}/ws"
        try:
            async with websockets.connect(uri, open_timeout=1.0, close_timeout=1.0, max_size=2**20) as websocket:
                first_frame = await asyncio.wait_for(websocket.recv(), timeout=1.0)
                envelope = json.loads(first_frame)
                assert envelope['type'] == 'snapshot'
                await websocket.send('{"type":"noop","payload":{}}')
                await asyncio.sleep(0.05)
        finally:
            hub.stop()

        assert received == ['{"type":"noop","payload":{}}']
        assert any(kind == 'listener_ready' for kind, _payload in reports)
        assert any(kind == 'listener_stopped' for kind, _payload in reports)
        assert hub.bound_port == listener['port']
