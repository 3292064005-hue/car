...
from robot_utils.constants import MODE_BOOT, MODE_IDLE, MODE_SAFE_STOP
...
class _Node:
    def __init__(self) -> None:
        ...
        self.navigation_route_pub = _Publisher()
        self.navigation_goal_id_pub = _Publisher()
        self.navigation_cancel_pub = _Publisher()
...
def test_runtime_supervision_fault_forces_safe_stop(monkeypatch) -> None:
    monkeypatch.setattr('robot_decision.decision_projection.command_capability_snapshot', lambda ctx: {'allowedTargetModes': [], 'modeReasons': {}, 'commandPermissions': {}})
    node = _Node()
    node.current_mode = MODE_IDLE
    service = _build_service(node)
    service.shutdown()
    msg = String()
    msg.data = '{"state":"unavailable","reasons":["decision_summary_timeout"],"components":{}}'
