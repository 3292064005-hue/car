...
def test_start_backend_skip_preflight_bypasses_gate(tmp_path: Path) -> None:
    ...
    subprocess.run(['bash', str(script), 'mock', '--skip-preflight', '--preflight-report-dir', str(report_dir)], cwd=str(repo_root), env=env, check=True)
...
def test_start_backend_hardware_does_not_implicitly_build_when_install_missing(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(['bash', str(script), 'hardware', '--preflight-report-dir', str(report_dir)], cwd=str(repo_root), env=env, text=True, capture_output=True)
