...
def test_runtime_param_patch_validates_as_one_batch() -> None:
    params = runtime_param_defaults()
    updated = apply_runtime_param_patch(params, patch={'maxLinearSpeed': 0.3, 'lowPowerThreshold': 30})
    assert updated['maxLinearSpeed'] == 0.3
    assert updated['lowPowerThreshold'] == 30
