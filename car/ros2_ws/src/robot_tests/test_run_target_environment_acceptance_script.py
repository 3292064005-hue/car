...
if [[ "$1" == "scripts/capture_target_environment_acceptance.py" ]]; then
  ...
  echo '{{"schemaVersion": 2, "artifactType": "target_environment_acceptance", "status": "ready"}}' > "$OUTPUT"
  exit 0
fi
echo python3:$* >> '{log_path}'
exit 0
...
''',
    ...
    subprocess.run(['bash', str(script), '--skip-npm-ci', '--allow-incomplete', '--output', str(output_path)], cwd=str(repo_root), env=env, check=True)
