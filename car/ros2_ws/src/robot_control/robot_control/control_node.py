...
from robot_control.arbiter import select_command_with_audit, zero_twist
...
class ControlNode(Node):
    def __init__(self, node_name: str = 'robot_control') -> None:
    ...
    def on_navigation(self, msg: Twist) -> None:
        """Cache one navigation-owned patrol command.

        Args:
            msg: Navigation velocity command projected for the patrol chain.
        ...
        Raises:
            None.
        """
        self._store_timed_twist(self.navigation_cmd, msg)
    ...
    def _publish_runtime_param_apply_result(self, payload: dict[str, object]) -> None:
        ...
        Raises:
            None. Publishing failures are logged but must not break control
            delivery while emitting status telemetry.
        """
        try:
            msg = String()
            ...
            try:
                self.get_logger().warning(f'failed to publish runtime parameter apply result error={exc}')
            except Exception:
                pass
            return
    ...
    def publish_final_cmd(self) -> None:
        ...
        self.selection.arbitration = arbitration
