...
from robot_navigation.navigation_model import Goal2D, RoutePlan, build_path, compute_navigation_command, load_route_plan
...
class Nav2AdapterNode(Node):
    """Separate navigation adapter lane for experimental provider activation.

    The node keeps the same provider-neutral surface as ``robot_navigation`` but
    lives in its own package so provider activation, rollback, launch selection,
    and future Nav2-oriented growth are isolated from the baseline provider.

    Runtime behavior:
        - ``backend_mode=local_adapter`` uses the in-repo straight-line tracker.
        - ``backend_mode=auto`` currently resolves to ``local_adapter`` unless a
          future backend integrator extends the node.

    This is intentionally a fully functional runtime lane rather than a
    isolated adapter package: route loading, goal tracking, cancel semantics, path
    preview, and status emission all execute inside this package.
    """
    ...
    def __init__(self, node_name: str = 'robot_navigation') -> None:
        ...
        backend_mode = str(self.get_parameter('backend_mode').value or 'auto').strip() or 'auto'
        self._backend_mode = 'local_adapter' if backend_mode in {'', 'auto', 'local_adapter'} else backend_mode
        self._backend_notes = self._resolve_backend_notes(self._backend_mode)
    ...
    def _resolve_backend_notes(self, backend_mode: str) -> dict[str, Any]:
        """Describe the currently selected backend runtime.

        Args:
            backend_mode: Requested backend mode.

        Returns:
            Serializable backend note block used in status payloads.

        Raises:
            None. Unsupported values fall back to ``local_adapter`` semantics.
        """
        selected = backend_mode if backend_mode == 'local_adapter' else 'local_adapter'
        return {
            'requestedBackend': backend_mode,
            'selectedBackend': selected,
            'nav2RuntimeAvailable': False,
            'fallbackApplied': selected != backend_mode,
            'notes': 'separate adapter lane is active; in-repo local adapter backend is selected',
        }
    ...
    def on_goal_pose(self, msg: PoseStamped) -> None:
        self._load_single_goal(
            Goal2D(x=float(msg.pose.position.x), y=float(msg.pose.position.y), label='pose_goal'),
            goal_id='pose_goal',
    ...
    def control_step(self) -> None:
        """Run one provider-lane control step.

        Returns:
            None.

        Raises:
            None. Invalid state collapses to a safe zero command.
        """
        ...
        self._publish_status_payload(self._payload('tracking', extra={
