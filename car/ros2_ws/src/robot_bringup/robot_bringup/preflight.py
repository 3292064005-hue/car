...
def _canonical_repo_root() -> Path:
    """Return the canonical Ubuntu-side workspace root."""
    return Path(__file__).resolve().parents[4]


def _repo_root() -> Path:
    """Return the externally exposed repo root for reports and compatibility checks.

    Boundary behavior:
        When the split snapshot exposes a legacy-compatible `ubuntu_side` alias,
        reports use that path so older tooling continues to resolve workspace
        assets correctly. The canonical workspace remains unchanged.
    """
    canonical = _canonical_repo_root()
    compatibility = canonical.parent / 'ubuntu_side'
    return compatibility if compatibility.exists() else canonical
...
def build_preflight_report(
    ...
    *,
    ...
    return {
        ...
        'runtime_supervision': profile.runtime_supervision_model(),
        'config_resolution': {
