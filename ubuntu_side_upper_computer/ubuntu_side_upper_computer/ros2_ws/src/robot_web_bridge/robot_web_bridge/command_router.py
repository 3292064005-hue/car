...
class CommandRouter:
    ...
    def _audit(self, event_id: str, command_type: str, status: str, message: str) -> None:
        self.node.audit_command(event_id, command_type, status, message)
    ...
    def _record_phase(self, event_id: str, command_type: str, phase: str, status: str, message: str, *, trace_id: str = '', extra: Mapping[str, Any] | None = None) -> None:
        recorder = getattr(self.node, 'record_command_phase', None)
        if recorder is None:
            return
    ...
    def _send_ack(self, event_id: str, command_type: str, lifecycle_status: str, message: str, *, trace_id: str = '', detail: str = '') -> None:
        ...
        self._audit(event_id, command_type, lifecycle_status, message)
        self.node.send_ack(
            event_id,
            legacy_status,
            message,
    ...
    def _update_task_state(self, payload: Mapping[str, Any]) -> None:
        store = getattr(self.node, 'state_store', None)
        if store is not None:
            store.update_mapping('task', payload)
            return
        self.node.state.task.update(payload)
        self.node._sync_snapshot_cache()

    def _deny(self, event_id: str, command_type: str, message: str, *, trace_id: str = '') -> None:
        self._record_phase(event_id, command_type, 'guard', 'denied', message, trace_id=trace_id)
        self._send_ack(event_id, command_type, 'denied', message, trace_id=trace_id)

    def _reject(self, event_id: str, command_type: str, message: str, *, trace_id: str = '') -> None:
        self._record_phase(event_id, command_type, 'execution', 'rejected', message, trace_id=trace_id)
        self._send_ack(event_id, command_type, 'rejected', message, trace_id=trace_id)

    def _wait_for_service(self, client: Any, *, name: str) -> bool:
        cache = getattr(self.node, 'readiness_cache', None)
        if cache is not None:
            try:
                return bool(cache.is_ready(name))
            except Exception:
                self.node.get_logger().warning(f'service readiness cache missing entry: {name}')
        for _ in range(SERVICE_WAIT_RETRIES):
            if client.wait_for_service(timeout_sec=SERVICE_WAIT_TIMEOUT_SEC):
                return True
        self.node.get_logger().warning(f'service unavailable after retries: {name}')
        return False
    ...
    def _wait_for_action_server(self, client: Any, *, name: str) -> bool:
        ...
        if cache is not None:
            try:
                ...
                self.node.get_logger().warning(f'action readiness cache missing entry: {name}')
        for _ in range(SERVICE_WAIT_RETRIES):
            if client.wait_for_server(timeout_sec=SERVICE_WAIT_TIMEOUT_SEC):
                return True
        self.node.get_logger().warning(f'action server unavailable after retries: {name}')
        return False


    def _register_timeout(self, future: Any, *, meta: PendingAck, kind: str, action_name: str = '') -> None:
        deadline = self._monotonic() + self._operation_timeout_sec
        self._pending_timeouts[id(future)] = PendingTimeout(
            future=future,
            meta=meta,
            kind=kind,
    ...
    def _clear_timeout(self, future: Any) -> PendingTimeout | None:
        return self._pending_timeouts.pop(id(future), None)

    def _cancel_goal_handle(self, goal_handle: Any, *, action_name: str) -> None:
        """Best-effort cancellation for timed-out action goals.

        Args:
            goal_handle: Goal handle returned by ``send_goal_async`` when accepted.
            action_name: Logical action name used for diagnostics.

        Returns:
            None.

        Raises:
            None. Cancellation failures are downgraded to warnings because the
            timeout path must still complete its local cleanup.
        """
        ...
        try:
            ...
            self.node.get_logger().warning(f'failed to cancel timed-out {action_name} goal: {exc}')

    def expire_pending(self, *, now_monotonic: float | None = None) -> int:
        now_value = self._monotonic() if now_monotonic is None else float(now_monotonic)
        expired = [entry for entry in list(self._pending_timeouts.values()) if entry.deadline_monotonic <= now_value]
        for entry in expired:
            self._pending_timeouts.pop(id(entry.future), None)
            self._expire_timeout_entry(entry)
        return len(expired)
